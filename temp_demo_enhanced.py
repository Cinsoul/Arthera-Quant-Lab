...
class IndicatorRequest(BaseModel):
    symbol: str = "AAPL"
    indicator_type: str = "sma"  # sma, ema, rsi, bollinger, macd
    period: int = 20
    data_points: int = 100
...
async def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        for symbol in request.symbols:
            ...
            for i in range(100):  # 100天的历史数据
                price_change = random.uniform(-0.05, 0.05)  # ±5%的日波动
        ...
        if request.strategy == "momentum":
            results = backtest_engine.run_momentum_strategy(historical_data, request.lookback_period)
        else:
            # 可以添加其他策略
            results = {"error": f"Strategy {request.strategy} not implemented"}
...
async def calculate_indicators(request: IndicatorRequest):
    ...
    try:
        ...
        for i in range(request.data_points):
        ...
        if request.indicator_type == "sma":
            indicator_values = tech_indicators.sma(prices, request.period)
        elif request.indicator_type == "ema":
            indicator_values = tech_indicators.ema(prices, request.period)
        elif request.indicator_type == "rsi":
            indicator_values = tech_indicators.rsi(prices, request.period)
        elif request.indicator_type == "bollinger":
            indicator_values = tech_indicators.bollinger_bands(prices, request.period)
        elif request.indicator_type == "macd":
            indicator_values = tech_indicators.macd(prices)
        else:
            return {
                "success": False,
                "error": f"Unknown indicator: {request.indicator_type}"
            }
        ...
        return {
            ...
            "raw_prices": prices[-len(indicator_values):] if isinstance(indicator_values, list) else prices,
...
@app.get("/strategies/performance-metrics")
async def get_performance_metrics():
    ...
    for strategy in strategies:
        # 模拟历史收益数据
        returns = [random.uniform(-0.05, 0.08) for _ in range(252)]  # 一年的日收益率
        equity_curve = [100000]
    ...
    return {
        ...
        "summary": {
            ...
            "best_sharpe": max(metrics, key=lambda x: x["sharpe_ratio"])["strategy_name"],
            "best_return": max(metrics, key=lambda x: x["total_return"])["strategy_name"],
            "lowest_drawdown": min(metrics, key=lambda x: x["max_drawdown"])["strategy_name"]
        }
    }
